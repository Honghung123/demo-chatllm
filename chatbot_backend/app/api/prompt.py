...
3. **Identify required tools** - Select only tools that provide missing information or capabilities
...
- `"order"`: Execution sequence number (starting from 1)
- `"reason"`: Brief explanation of why this tool is needed given the history
...
## Critical Rules
- Return ONLY the JSON array (no markdown, explanations, or additional text)
- Use exact tool names from available tools list
""" + f"""
- Include username: '{username}' and role: '{role}' when tools require them
- Use `classify_file_based_on_content` (not classify_text/classify_document) for file classification
- If request cannot be fulfilled: `[{{"error": "explanation"}}]`
- If no new tools needed: `[{{"message": "Using existing context from conversation history"}}]`

## Available Tools Reminder
Remember these key tools:
- `classify_file_based_on_content`: For file content classification
